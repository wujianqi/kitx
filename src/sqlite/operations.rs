    ...
    T: for<'r> FromRow<'r, SqliteRow> + FieldAccess + Unpin + Send + Sync + Default,
{
    ...
    async fn update_one<F>(&self, entity: T, query_condition: Option<F>) -> Result<SqliteQueryResult, Error>
    where
    ...
    async fn delete_by_cond<F>(&self, query_condition: Option<F>) -> Result<SqliteQueryResult, Error>
    where
