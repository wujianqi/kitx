...
where
    T: for<'r> FromRow<'r, SqliteRow> + FieldAccess + Default + Clone + Debug + Unpin + Send + Sync,
{
    
    table_query: CompositeKeyTable<'a, T, DataKind<'a>, Sqlite, DataKind<'a>> ,
    query: Arc<SqliteQuery<'a>>,
    ...
    T: for<'r> FromRow<'r, SqliteRow> + FieldAccess + Default + Clone + Debug  + Unpin + Send + Sync,
{
    ...
    pub fn new(table_name: &'a str, primarys: Vec<&'a str>) -> Self 
    {
        ...
        Operations { 
            table_query, 
            query: Arc::new(SqliteQuery::new()), 
            _phantom: PhantomData 
        }
    }

    /// Sets the query for the operations.
    pub fn set(mut self, query: Arc<SqliteQuery<'a>>) -> Self {
        self.query = query;
        self
    }
}

impl<'a, T> OpsActionTrait<'a, T, Sqlite, DataKind<'a>> for Operations<'a, T>
where
    T: for<'r> FromRow<'r, SqliteRow> + FieldAccess + Unpin + Send + Sync + Default + Clone + Debug,
{
    ...
    async fn update_one(&self, entity: T) -> Result<SqliteQueryResult, Error>
    {
        let builder = self.table_query.update_one(entity)?;
        self.query.execute(builder).await
    }

    async fn update_by_cond<F>(&self, query_condition: F) -> Result<SqliteQueryResult, Error>
    where
    ...
    async fn delete_by_pk(&self, key: impl Into<PrimaryKey<DataKind<'a>>> + Send + Sync) -> Result<SqliteQueryResult, Error> {
   ...
   async fn get_list_paginated<F>(
    ...
    async fn restore_by_pk(&self, key: impl Into<PrimaryKey<DataKind<'a>>> + Send + Sync) -> Result<SqliteQueryResult, Error> {
        let builder = self.table_query.restore_by_pk(key)?;
        self.query.execute(builder).await
    }

    async fn restore_by_cond<F>(&self, query_condition: F) -> Result<SqliteQueryResult, Error>
    where
        F: Fn(&mut Self::UpdateFilter<'a>) + Send + Sync,
    {
