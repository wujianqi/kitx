    ...
    T: for<'r> FromRow<'r, SqliteRow> + FieldAccess + Unpin + Send + Sync + Default,
{
    ...
    pub async fn insert_one(&self, entity: T) -> Result<SqliteQueryResult, Error> {
        let builder = self.table_query.insert_one(entity)?;
        self.query.execute(builder).await
    }

    pub async fn insert_many(&self, entities: Vec<T>) -> Result<SqliteQueryResult, Error> {
        let builder = self.table_query.insert_many(entities)?;
        self.query.execute(builder).await
    }

    pub async fn delete_by_keys(&self, keys: &[(&str, DataKind<'a>)]) -> Result<SqliteQueryResult, Error> {
    ...
    pub async fn delete_by_cond<F>(&self, query_condition: F) -> Result<SqliteQueryResult, Error>
    where
    ...
    pub async fn restore_one(&self, key: impl Into<DataKind<'a>> + Send) -> Result<SqliteQueryResult, Error> {
        let builder = self.table_query.restore_one(key)?;
        self.query.execute(builder).await
    }

    pub async fn restore_many(&self, keys: Vec<impl Into<DataKind<'a>> + Send>) -> Result<SqliteQueryResult, Error> {
