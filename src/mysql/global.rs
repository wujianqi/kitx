...
static MYSQL_G_S_D_F: OnceLock<(&'static str, Vec<&'static str>)> = OnceLock::new();
...
thread_local! {
    static MYSQL_G_F_S: Cell<Option<(Expr<DataKind<'static>>, Vec<&'static str>)>> = Cell::new(None);
}
...
pub fn set_global_filter(filter: Expr<DataKind<'static>>, exclude_tables: Vec<&'static str>) {
...
pub fn get_global_filter() -> Option<(Expr<DataKind<'static>>, Vec<&'static str>)> {
    MYSQL_G_F_S.with(|cell| cell.take())
}