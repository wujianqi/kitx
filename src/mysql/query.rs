...
impl<'a> MySqlQuery<'a>  {
    ...
    pub fn shared() -> Shared<MySqlQuery<'a>> {
        Shared::new(Self::new())
    }

    async fn execute_with_trans(&self, 
        pending_statements: Vec<(String, Vec<DataKind<'a>>)>) -> Result<Vec<MySqlQueryResult>, Error>
    {
    ...
    pub async fn commit(&self) -> Result<Vec<MySqlQueryResult>, Error> {
        let builders = {
...
impl<'a> QueryExecutor<DataKind<'a>, MySql> for MySqlQuery<'a> {
