    ...
    T: for<'r> FromRow<'r, MySqlRow> + FieldAccess + Unpin + Send + Sync + Default,
{
    ...
    async fn update_by_key(&self, entity: T) -> Result<MySqlQueryResult, Error> {
        let builder = self.table_query.update_by_key(entity)?;
        self.query.execute(builder).await
    }

    async fn update_one<F>(&self, entity: T, query_condition: Option<F>) -> Result<MySqlQueryResult, Error>
    where
    ...
    async fn delete_by_cond<F>(&self, query_condition: Option<F>) -> Result<MySqlQueryResult, Error>
    where
    ...
    async fn restore_one(&self, key: impl Into<DataKind<'a>> + Send) -> Result<MySqlQueryResult, Error> {
        let builder = self.table_query.restore_one(key)?;
        self.query.execute(builder).await
    }

    async fn restore_many(&self, keys: Vec<impl Into<DataKind<'a>> + Send>) -> Result<MySqlQueryResult, Error> {
