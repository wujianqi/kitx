    ...
    T: for<'r> FromRow<'r, MySqlRow> + FieldAccess + Unpin + Send + Sync + Default,
{
        ...
        F: FnOnce(&mut Self::QueryFilter<'a>) + Send + 'a
    {
        ...
        if page_size > 1000 {
            return Err(OperationError::new("Page size cannot exceed 1000".to_string()));
        }
        ...
        F: FnOnce(&mut Self::QueryFilter<'a>) + Send + 'a
    {
        ...
        if limit > 500 {
            return Err(OperationError::new("Limit cannot exceed 500".to_string()));
        }
    ...
    async fn upsert_one(&self, entity: T) -> Result<Self::QueryResult, Error> {
        ...
        let builder = Insert::into(self.table_name)
            .columns(&cols_names)
            .values(vec![cols_values])
            .on_duplicate_key_update(&cols_names);
