...
where 
    T: for<'r> FromRow<'r, MySqlRow> + FieldAccess + Unpin + Send, // 使用 MySqlRow
{
    /// 实体对象，表示要操作的数据记录。
    entity: T,
    /// 表名，表示实体对应的数据库表。
...
where 
    T: for<'r> FromRow<'r, MySqlRow> + FieldAccess + Unpin + Send + Sync, // 使用 MySqlRow
{
    /// 创建一个新的 `EntityOperations` 实例。
    fn new(entity: T, table_name: &'a str, key_name: &'a str) -> Self {
        EntityOperations {
    ...
    async fn insert(self) -> Result<i64, Error> {
        ...
        Ok(result.last_insert_id() as i64) // MySQL 使用 last_insert_id
    }
    ...
    async fn insert_many(self, entities: Vec<T>) -> Result<Vec<i64>, Error> {
        ...
        Ok((0..result.rows_affected()).map(|_| result.last_insert_id() as i64).collect())
    }
