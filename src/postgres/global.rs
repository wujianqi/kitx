...
static POSTGERS_G_S_D_F: OnceLock<(&'static str, &'static [&'static str])> = OnceLock::new();
...
pub fn set_global_soft_delete_field(field_name: &'static str, exclude_tables: &'static [&'static str]) {
    POSTGERS_G_S_D_F.get_or_init(|| (field_name, exclude_tables));
}
...
pub fn get_global_soft_delete_field() -> Option<&'static (&'static str, &'static [&'static str])> {
    POSTGERS_G_S_D_F.get()
}

thread_local! {
    static POSTGERS_G_F_S: OnceCell<Option<(
