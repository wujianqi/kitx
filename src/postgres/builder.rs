...
mod tests {
    ...
    use crate::{
        ...
        postgres::{builder::{Delete, Insert, Select, Subquery, Update, Upset, QB}, 
    ...
    async fn test_find_list_paginated() {
        ...
        let total = fetch_scalar(qb2).await.unwrap();

        let pr = PaginatedResult::new(list, total as u64, 1, 10);
        dbg!(pr);
    }
